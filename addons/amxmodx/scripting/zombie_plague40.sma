...
// Plugin Version
new const PLUGIN_VERSION[] = "v4.3 Termux"

// Natives
native players_menu(id)
native privileges_menu(id)
...
show_menu_game(id)
{
	...
	static menu[850], len, userflags
	len = 0
	userflags = get_user_flags(id)
	
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "^^1|• | ^^2Termux Zombie Plague 4.3 ^^1|• |^n^^1|• | ^^3Dev: ^^2PowerSiderS.X DARK ^^1|• |^n^n")
	
	// 1. Buy weapons
	if (get_pcvar_num(cvar_buycustom))
		len += formatex(menu[len], charsmax(menu) - len, "^^1|1 |. ^^2%L^n", id, "MENU_BUY")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|1 |. \d%L^n", id, "MENU_BUY")
	
	// 2. Extra items
	if (get_pcvar_num(cvar_extraitems) && g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "^^1|2 |. ^^2%L^n", id, "MENU_EXTRABUY")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|2 |. \d%L^n", id, "MENU_EXTRABUY")
	
	// 3. Zombie class
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "^^1|3 |. ^^2%L^n", id,"MENU_ZCLASS")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|3 |. \d%L^n", id,"MENU_ZCLASS")
	
	// 4. Unstuck
	if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "^^1|4 |. ^^2%L^n^n", id, "MENU_UNSTUCK")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|4 |. \d%L^n^n", id, "MENU_UNSTUCK")
	
	// 5. Player Menu
	if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "^^1|5 |. ^^2Player Menu^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|5 |. \dPlayer Menu^n")
	
	// 6. Privileges Menu
	if (g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "^^1|6 |. ^^2Privileges Menu^n^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|6 |. \dPrivileges Menu^n^n")
		
	// 7. Join spec
	if (!g_isalive[id] || !get_pcvar_num(cvar_blocksuicide) || (userflags & g_access_flag[ACCESS_ADMIN_MENU]))
		len += formatex(menu[len], charsmax(menu) - len, "^^1|7 |. ^^2%L^n^n", id, "MENU_SPECTATOR")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|7 |. \d%L^n^n", id, "MENU_SPECTATOR")
	
	// 9. Admin menu
	if (userflags & g_access_flag[ACCESS_ADMIN_MENU])
		len += formatex(menu[len], charsmax(menu) - len, "^^1|9 |. ^^2%L", id, "MENU_ADMIN")
	else
		len += formatex(menu[len], charsmax(menu) - len, "^^1|9 |. \d%L", id, "MENU_ADMIN")
	
	// 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^^1|0 |. ^^2%L", id, "MENU_EXIT")
...
public show_menu_buy1(taskid)
{
	...
	static menu[900], len, weap, maxloops
	len = 0
	maxloops = min(WPN_STARTID+7, WPN_MAXIDS)
	
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "^^1|• | ^^2Termux ZM Mod ^^1|• |^n^^1|• | ^^2%L ^^1[%d-%d] |• |^n^n", id, "MENU_BUY1_TITLE", WPN_STARTID+1, min(WPN_STARTID+7, WPN_MAXIDS))
	
	// 1-7. Weapon List
	for (weap = WPN_STARTID; weap < maxloops; weap++)
		len += formatex(menu[len], charsmax(menu) - len, "^^1|%d |. ^^2%s^n", weap-WPN_STARTID+1, WEAPONNAMES[ArrayGetCell(g_primary_weaponids, weap)])
	
	// 8. Auto Select
	len += formatex(menu[len], charsmax(menu) - len, "^n^^1|8 |. ^^2%L ^^1[^^5%L^^1]", id, "MENU_AUTOSELECT", id, (WPN_AUTO_ON) ? "MOTD_ENABLED" : "MOTD_DISABLED")
	
	// 9. Next/Back - 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^^1|9 |. ^^2%L/%L^n^n^^1|0 |. ^^2%L", id, "MENU_NEXT", id, "MENU_BACK", id, "MENU_EXIT")
...
show_menu_buy2(id)
{
	...
	static menu[850], len, weap, maxloops
	len = 0
	maxloops = ArraySize(g_secondary_items)
	
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "^^1|• | ^^2Termux ZM Mod ^^1|• |^n^^1|• | ^^2%L ^^1|• |^n", id, "MENU_BUY2_TITLE")
	
	// 1-6. Weapon List
	for (weap = 0; weap < maxloops; weap++)
		len += formatex(menu[len], charsmax(menu) - len, "^n^^1|%d |. ^^2%s", weap+1, WEAPONNAMES[ArrayGetCell(g_secondary_weaponids, weap)])
	
	// 8. Auto Select
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^^1|8 |. ^^2%L ^^1[^^5%L^^1]", id, "MENU_AUTOSELECT", id, (WPN_AUTO_ON) ? "MOTD_ENABLED" : "MOTD_DISABLED")
...
show_menu_extras(id)
{
	...
	static menuid, menu[428], item, team, buffer[32]
	
	// Title
	formatex(menu, charsmax(menu), "^^1|• | ^^2%L ^^1[%L] |• |", id, "MENU_EXTRA_TITLE", id, g_zombie[id] ? g_nemesis[id] ? "CLASS_NEMESIS" : "CLASS_ZOMBIE" : g_survivor[id] ? "CLASS_SURVIVOR" : "CLASS_HUMAN")
	...
	for (item = 0; item < g_extraitem_i; item++)
	{
		...
		formatex(menu, charsmax(menu), "^^1|• | ^^2%s ^^5%d ^^1%L", buffer, ArrayGetCell(g_extraitem_cost, item), id, "AMMO_PACKS2")
		buffer[0] = item
		buffer[1] = 0
...
public show_menu_zclass(id)
{
	...
	static menuid, menu[428], class, buffer[32], buffer2[32]
	
	// Title
	formatex(menu, charsmax(menu), "^^1|• | ^^2%L ^^1|• |", id, "MENU_ZCLASS_TITLE")
	...
	for (class = 0; class < g_zclass_i; class++)
	{
		...
		if (class == g_zombieclassnext[id])
			formatex(menu, charsmax(menu), "\d%s ^^1%s", buffer, buffer2)
		else
			formatex(menu, charsmax(menu), "^^2%s ^^1%s", buffer, buffer2)
...
{
	// Show mod info
	zp_colored_print(0, "^^1**** ^^2Termux Zombie Plague V2 ^^1****")
	zp_colored_print(0, "^^1**** ^^2By - ^^5PowerSiderS.X DARK^^1****")
	zp_colored_print(0, "^^1**** ^^2YouTube: ^^5PowerSiderS CS ^^1****")
...
public ShowHUD(taskid)
{
	...
	else
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "-[ %L: %d ]-^n-[ %L %s ]-^n-[ %L %d ]-^n-[ Level: 0/50 ]-^n-[ XP: 0/90 ]-^n-[ YT: PowerSiderS CS ]-", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD])
	}
}
